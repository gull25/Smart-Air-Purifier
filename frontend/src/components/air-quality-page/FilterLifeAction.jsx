...
const FilterLifeAction = ({ data }) => {
  ...
  // Calculate dynamic days based on 180-day standard lifespan
  const MAX_DAYS = 180;
  const currentEfficiency = data?.overallEfficiency || 100;
  const daysRemaining = Math.max(0, Math.round((currentEfficiency / 100) * MAX_DAYS));
  
  const estDate = new Date();
  estDate.setDate(estDate.getDate() + daysRemaining);
  const formattedDate = estDate.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
    ...
    <div className="lg:col-span-4 bg-surface-container-lowest rounded-2xl p-space-xl shadow-sm flex flex-col justify-between">
      ...
      <div className="my-space-lg flex flex-col items-center justify-center text-center p-space-md bg-surface-container-low rounded-xl">
        <div className="flex items-baseline gap-1">
          <span className="font-telemetry-value-lg text-telemetry-value-lg text-primary tracking-tight">{daysRemaining}</span>
          <span className="font-headline-sm text-headline-sm text-on-surface">Days</span>
        </div>
        <span className="font-body-md text-body-md text-on-surface-variant mt-1">Est. replacement: {formattedDate}</span>
        <div className="w-full bg-surface-container-highest h-2 rounded-full overflow-hidden mt-space-md">
          <div className="bg-primary h-full rounded-full transition-all duration-1000" style={{ width: `${currentEfficiency}%` }}></div>
        </div>
      </div>
